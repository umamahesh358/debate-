...
Respond in JSON format:
{
  ...
  "improvements": ["improvement1", "improvement2", "..."]
      ...
      `;
            ...
            logger_1.logger.error('Error in analyzeArgument:', error);
            // Fallback analysis
            return this.getFallbackAnalysis();
        }
    }
    // Generate AI opponent response
    static async generateResponse(userInput, topic, role, previousArguments = []) {
        try {
...
Respond in JSON format:
{
  ...
  "strategy": "Brief strategy description"
      ...
      `;
            ...
            logger_1.logger.info(`AI response generated for topic: ${topic}`);
            return {
            ...
            logger_1.logger.error('Error in generateResponse:', error);
            // Fallback response
            return this.getFallbackResponse(topic, role);
        }
    }
    // Analyze video for presentation skills
    static async analyzeVideo(videoBuffer, metadata) {
        try {
            ...
            logger_1.logger.error('Error in analyzeVideo:', error);
            // Fallback analysis
            return {
                eyeContact: 75,
                posture: 80,
    ...
    static async generateCoachingTip(context) {
        try {
            const prompt = `
...
Respond in JSON format:
{
  ...
  "immediate": true/false
      ...
      `;
            ...
            logger_1.logger.info(`Coaching tip generated for situation: ${context.situation}`);
            return {
            ...
            logger_1.logger.error('Error in generateCoachingTip:', error);
...
Respond in JSON format:
{
  ...
  "nextSteps": ["step1", "step2", "step3"]
      ...
      `;
            ...
            logger_1.logger.info(`Comprehensive feedback generated for session`);
            return feedback;
        }
        catch (error) {
            logger_1.logger.error('Error in generateFeedback:', error);
            // Fallback feedback
    ...
    // Fallback response
    static getFallbackResponse(topic, role) {
        const opponentRole = role === 'government' ? 'opposition' : 'government';
        return {
    ...
    // Fallback coaching tip
    static getFallbackCoachingTip(situation) {
        const tips = {
            opening: {
                type: 'strategy',
        ...
        return tips[situation] || tips.delivery;
    }
    // Fallback feedback
    static getFallbackFeedback() {
        return {
            overallScore: 70,
            argumentQuality: {
                score: 70,
    ...
    static generateVideoFeedback(eyeContact, posture, gestures, engagement, speakingPace) {
        const feedback = [];
        if (eyeContact < 70) {
