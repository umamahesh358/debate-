...
class LearningService {
    ...
    static async getModules(filters, userId, page = 1, limit = 20) {
        const where = {
            isActive: true
        };
        if (filters.level) {
        ...
        const [modules, total] = await Promise.all([
            prisma_1.prisma.learningModule.findMany({
                where,
                skip,
                take: limit,
                orderBy: [
            ...
            prisma_1.prisma.learningModule.count({ where })
        ]);
        // If user ID provided, get user's progress for these modules
        let userProgress = [];
        if (userId) {
            const moduleIds = modules.map(m => m.id);
            userProgress = await prisma_1.prisma.learningProgress.findMany({
                where: {
                    userId,
    ...
    static async getModuleById(moduleId, userId) {
        const module = await prisma_1.prisma.learningModule.findUnique({
        ...
        if (userId) {
            userProgress = await prisma_1.prisma.learningProgress.findUnique({
                where: {
                    userId_moduleId: {
                        userId,
                        moduleId
        ...
        if (userId) {
            const allUserProgress = await prisma_1.prisma.learningProgress.findMany({
                where: { userId },
                select: {
                    moduleId: true,
                    status: true,
    ...
    static isModuleLocked(module, userProgress, userId) {
        if (!userId)
            return false;
    ...
    static async getLearningPath(userId) {
        ...
        const completedModules = await prisma_1.prisma.learningProgress.findMany({
            where: {
                userId,
                status: 'completed'
            },
        ...
        const availableModules = await this.getModules({ level: currentLevel }, userId, 1, 10);
        // Get modules from next level as well
        const nextLevelModules = await this.getModules({ level: currentLevel + 1 }, userId, 1, 5);
        return {
    ...
    static async updateProgress(userId, data) {
        ...
        const existingProgress = await prisma_1.prisma.learningProgress.findUnique({
            where: {
                userId_moduleId: {
                    userId,
        ...
        if (existingProgress) {
            ...
            if (data.completion !== undefined)
                updateData.completion = data.completion;
            ...
            const updatedProgress = await prisma_1.prisma.learningProgress.update({
                where: {
                    userId_moduleId: {
                        userId,
                        moduleId: data.moduleId
            ...
            if (data.status === 'completed' && existingProgress.status !== 'completed') {
                await prisma_1.prisma.learningModule.update({
            ...
            logger_1.logger.info(`Learning progress updated: ${data.moduleId} for user: ${userId}`);
            return updatedProgress;
        }
        else {
            // Create new progress record
            const newProgress = await prisma_1.prisma.learningProgress.create({
                data: {
                    userId,
                    moduleId: data.moduleId,
            ...
            logger_1.logger.info(`New learning progress created: ${data.moduleId} for user: ${userId}`);
            return newProgress;
        }
    }
    // Get user's learning progress
    static async getUserProgress(userId, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const [progress, total] = await Promise.all([
            prisma_1.prisma.learningProgress.findMany({
                where: { userId },
                skip,
                take: limit,
            ...
            prisma_1.prisma.learningProgress.count({ where: { userId } })
        ]);
        return {
            progress,
            pagination: {
    ...
    static async getNextModule(userId) {
        ...
        if (recommendedModules.length === 0) {
            ...
            const nextLevelModules = await this.getModules({ level: learningPath.currentLevel + 1 }, userId, 1, 1);
    ...
    static calculateTimeToNextLevel(currentLevel, currentExperience) {
    ...
    static async getLearningStats(userId) {
        const stats = await prisma_1.prisma.learningProgress.aggregate({
            where: { userId },
            _count: {
                id: true
            },
        ...
        const completedStats = await prisma_1.prisma.learningProgress.aggregate({
        ...
        const levelStats = await prisma_1.prisma.learningProgress.groupBy({
            by: ['module'],
            where: { userId },
            _count: true,
            _avg: {
        ...
        const recentActivity = await prisma_1.prisma.learningProgress.findMany({
            where: {
                userId,
