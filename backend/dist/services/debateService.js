...
class DebateService {
    ...
    static async getTopics(filters, page = 1, limit = 20) {
        const where = {
            isActive: true
        };
        // Apply filters
        ...
        const [topics, total] = await Promise.all([
            prisma_1.prisma.debateTopic.findMany({
                where,
                skip,
                take: limit,
                orderBy: [
            ...
            prisma_1.prisma.debateTopic.count({ where })
        ]);
        return {
            topics,
            pagination: {
    ...
    static async getTopicById(topicId) {
        const topic = await prisma_1.prisma.debateTopic.findUnique({
    ...
    // Get random topic
    static async getRandomTopic(filters) {
        const where = {
            isActive: true
        };
        if (filters) {
        ...
        const count = await prisma_1.prisma.debateTopic.count({ where });
        if (count === 0) {
            throw new Error('No topics found matching the criteria');
        }
        // Get random topic
        const randomIndex = Math.floor(Math.random() * count);
        const topics = await prisma_1.prisma.debateTopic.findMany({
            where,
            skip: randomIndex,
            take: 1,
            select: {
    ...
    static async createSession(data) {
        ...
        const session = await prisma_1.prisma.debateSession.create({
            data: {
                userId: data.userId,
        ...
        await prisma_1.prisma.debateTopic.update({
        ...
        logger_1.logger.info(`Debate session created: ${session.id} for user: ${data.userId}`);
        return session;
    }
    ...
    static async getSessionById(sessionId, userId) {
        ...
        const session = await prisma_1.prisma.debateSession.findFirst({
            where,
            include: {
                topic: {
                    select: {
    ...
    static async updateSession(sessionId, userId, data) {
        ...
        const existingSession = await prisma_1.prisma.debateSession.findFirst({
        ...
        const updatedSession = await prisma_1.prisma.debateSession.update({
            where: { id: sessionId },
            data: updateData,
            include: {
                topic: {
        ...
        logger_1.logger.info(`Debate session updated: ${sessionId}`);
    ...
    static async getUserSessions(userId, page = 1, limit = 20) {
        ...
        const [sessions, total] = await Promise.all([
            prisma_1.prisma.debateSession.findMany({
                where: { userId },
                skip,
                take: limit,
            ...
            prisma_1.prisma.debateSession.count({ where: { userId } })
        ]);
        return {
            sessions,
            pagination: {
    ...
    static calculateScore(metrics, difficulty) {
        let baseScore = 0;
    ...
    static async getDebateStats(userId) {
        ...
        const roleStats = await prisma_1.prisma.debateSession.groupBy({
            by: ['role'],
            where: { userId, completedAt: { $ne: null } },
            _count: {
                id: true
        ...
        const difficultyStats = await prisma_1.prisma.debateSession.groupBy({
            by: ['topic'],
            where: { userId, completedAt: { $ne: null } },
            _count: true,
            _avg: {
        ...
        const recentSessions = await prisma_1.prisma.debateSession.findMany({
            where: {
                userId,
                completedAt: { $ne: null },
